...
class CacheServer:
    ...
    def set_json_if_new(self, key, value, expire_time=None):
        """
        Set a JSON object in the cache if it's different from the previously stored JSON object.

        :param key: The key to set.
        :param value: The JSON object to store.
        :param expire_time: Optional expiration time for the key in seconds.
        """
        existing_value = self.get_json(key)
        if existing_value != value:
            self.set_json(key, value, expire_time)

    def hash_json(self, value):
        """
        Generate a hash of a JSON object to compare with other JSON objects.

        :param value: The JSON object to hash.
